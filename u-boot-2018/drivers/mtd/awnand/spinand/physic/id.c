...
struct aw_spinand_phy_info xcsp[] =
{
	#if 1
	{
		...
		.DieCntPerChip  = 1,
		.SectCntPerPage = 4,//(一页有多少个扇区,2048/512)
		.PageCntPerBlk  = 64,//每个block有多个页(每个block包含多少页,128*1024/2048)
		.BlkCntPerDie	= 2048,//一个die有多少个block(256*1024/128 )
		.OobSizePerPage = 64,//每个页有多少个obb区域
		...
		.SectCntPerPage = 4,//(一页有多少个扇区,2048/512)
		.PageCntPerBlk  = 64,//每个block有多个页(每个block包含多少页,128*1024/2048)
		.BlkCntPerDie	= 1024,//一个die有多少个block(256*1024/128 )
		.OobSizePerPage = 64,//每个页有多少个obb区域
		.OperationOpt	= SPINAND_QUAD_READ | SPINAND_QUAD_PROGRAM | SPINAND_DUAL_READ,
		.MaxEraseTimes  = 100000,
		.EccType	= BIT2_LIMIT1_ERR2,
	...
	.DieCntPerChip  = 1,
	.SectCntPerPage = 8,//(一页有多少个扇区,4096/512)
	.PageCntPerBlk  = 64,//每个block有多个页(每个block包含多少页,512*1024/2048)
	.BlkCntPerDie	= 2048,//一个die有多少个block(512*1024/256 )
	.OobSizePerPage = 64,//每个页有多少个obb区域
	.OperationOpt	= SPINAND_QUAD_READ | SPINAND_QUAD_PROGRAM | SPINAND_DUAL_READ,
	.MaxEraseTimes  = 100000,
	.ecc_status_shift = ECC_STATUS_SHIFT_4,
...
struct aw_spinand_phy_info mxic[] =
{
	...
	{
		.Model		= "MX35LF4GE4AD",
		.NandID 	= {0xc2, 0x37, 0x03, 0xff, 0xff, 0xff, 0xff, 0xff},
		.DieCntPerChip	= 1,
		.SectCntPerPage = 8,
		.PageCntPerBlk	= 64,
		.BlkCntPerDie	= 2048,
...
#define BAD_BLK_MARK_POS4 "last_2_page"
	...
	if (ret < 0 || NULL == bad_blk_mark_pos) {
		...
		if (!memcmp(bad_blk_mark_pos, BAD_BLK_MARK_POS1, strlen(BAD_BLK_MARK_POS1)))
			...
			info.BadBlockFlag = BAD_BLK_FLAG_FIRST_2_PAGE;
